// InjectMe
#include "Asda2Encryption.hpp"

using namespace InjectMe::Utils;
using namespace InjectMe::Network;

sf::Uint8 Asda2EncryptionKey[] =
{
	
};

Asda2Encryption *Asda2Encryption::s_instance = nullptr;

Asda2Encryption* Asda2Encryption::Instance()
{
	if (s_instance == nullptr)
		s_instance = new Asda2Encryption();

	return s_instance;
}

void Asda2Encryption::Encrypt(Buffer &buffer)
{
	// —читывание длины пакета
	sf::Uint16 packetLength;
	buffer.Read(1, packetLength);

	// —читывание ключа шифровани€ пакета
	sf::Uint8 packetKey;
	buffer.Read(3, packetKey);

	// Ўифрование пакета
	Encrypt(buffer.GetMemory().lpAddress + 4, packetKey, 4, packetLength - 5);
}

void Asda2Encryption::Encrypt(void *data, sf::Uint8 key, size_t offset, size_t length)
{
	auto buffer = reinterpret_cast<char*>(data);
	for (size_t i = 0; i < length; i++)
		buffer[i] ^= Asda2EncryptionKey[2 * Asda2EncryptionKey[247]] ^ Asda2EncryptionKey[key * 2 + 1] ^ Asda2EncryptionKey[(255 - (i - 4 + offset) % 256) * 2 + 1];
}