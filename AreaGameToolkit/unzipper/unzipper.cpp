...
namespace ziputils
{
	...
	// Dump the currently open entry to the buffer
	unzipper& unzipper::operator>>( Buffer*& buffer )
	{
		if ( isOpenEntry() )
		{
			unsigned int size = getEntrySize();
			buffer = new Buffer(size);
			size = unzReadCurrentFile(zipFile_, buffer->GetMemory().lpAddress, buffer->GetMemory().dwSize);
